...
function (_React$Component) {
  ...
  function Podcasts(props) {
    ...
    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "search", function (e) {
      ...
      var input = e.target.value.replace(/ /g, '').toLowerCase();
      podcasts.forEach(function (podcast) {
        // Podcast content to search for (podcast title)
        var content = podcast.acf.podcast_title.replace(/ /g, '').toLowerCase();
        console.log(input, content); // If podcast doesn't contain user input, remove from view

        podcast.visible = content.includes(input) ? true : false;
      });

      _this.setState(podcasts);
    });
...
//# sourceMappingURL=podcasts.js.cf1e1524420b475bd530.hot-update.js.map